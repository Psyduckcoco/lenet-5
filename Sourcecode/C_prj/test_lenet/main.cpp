...
//#define is_convert_2bin //是否转到二进制格式
...
int main()
{
	...
	{
		sprintf_s(str2read_func, "..\\..\\dataset\\test\\%d.jpg", i);
		...
		for (int row = 0; row < 32; row++)      //行
		{
			...
			{
				pic_in[i][row * 32 + col] = (img_in.at<uchar>(row, col)) / 255.0; //归一化到0 ~ 1 
			}
		}
	}
	sprintf_s(str2read_func, "..\\..\\dataset\\vali_data");
	print2files(str2read_func, &pic_in[0][0], 8952 * 32 * 32);
	...
	for(int i=0;i<50;i++)
		cout << " " << err_location[i] ;
	cout << endl << "Recognition rate is " << (  1-   float(err_cnt) / 8952.0  )   * 100 << '%' << endl;
...
void read_parameters()
{
	...
	read_bin_float("..\\..\\python_code\\parameter\\conv1_w", &W_CONV1[0][0][0][0], 6 * 5 * 5);
	read_bin_float("..\\..\\python_code\\parameter\\conv3_w", &W_CONV3[0][0][0][0], 16 * 6 * 5 * 5);
	read_bin_float("..\\..\\python_code\\parameter\\conv5_w", &W_CONV5[0][0][0][0], 120 * 16 * 5 * 5);

	read_bin_float("..\\..\\python_code\\parameter\\fc6_w", &WFC6[0], 10080);
	read_bin_float("..\\..\\python_code\\parameter\\fc7_w", &WFC7[0], 840);

	read_bin_float("..\\..\\python_code\\parameter\\b_conv1", &b_conv1[0], 6);
	read_bin_float("..\\..\\python_code\\parameter\\b_conv3", &b_conv3[0], 16);
	read_bin_float("..\\..\\python_code\\parameter\\b_conv5", &b_conv5[0], 120);
	read_bin_float("..\\..\\python_code\\parameter\\b_fc6", &b_fc6[0], 84);
	read_bin_float("..\\..\\python_code\\parameter\\b_fc7", &b_fc7[0], 10);

	read_bin_int("..\\..\\python_code\\parameter\\test_label", &labels[0], 8952);
